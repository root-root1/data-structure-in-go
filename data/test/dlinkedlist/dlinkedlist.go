...
func (linkedlist *DLinkedList) NodeBetweenValue(firstData int, secondData int) *Node {
	var node *Node
	var withValue *Node
...
func (linkedlist *DLinkedList) AddTOEnd(data int) {
	...
	node.data = data
	node.nextNode = nil
	var lastValue = linkedlist.lastNode()

	if lastValue != nil {
		lastValue.nextNode = node
		node.prevNode = lastValue
	}
}
